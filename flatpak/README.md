...
flatpak build-bundle --runtime-repo=https://flathub.org/repo/flathub.flatpakrepo flatpak-repo Agent-Switch-Linux.flatpak com.agentswitch.desktop
```

4) Install and run:

```bash
flatpak install --user ./Agent-Switch-Linux.flatpak
...
```yaml
  - --filesystem=~/.agent-switch:create
