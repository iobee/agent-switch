...
### 🌐 Die einzige offizielle Website: **[github.com/iobee/agent-switch](https://github.com/iobee/agent-switch)**
...
Agent Switch für macOS ist von Apple code-signiert und notarisiert. Sie können es direkt herunterladen und installieren — es sind keine zusätzlichen Schritte erforderlich. Wir empfehlen die Verwendung des `.dmg`-Installationsprogramms.
...
- **Datenbank**: `~/.agent-switch/agent-switch.db` (SQLite — Anbieter, MCP, Prompts, Skills)
- **Lokale Einstellungen**: `~/.agent-switch/settings.json` (gerätebezogene UI-Einstellungen)
- **Backups**: `~/.agent-switch/backups/` (automatisch rotiert, behält die 10 neuesten)
- **Skills**: `~/.agent-switch/skills/` (standardmäßig per Symlink mit den entsprechenden Apps verbunden)
- **Skill-Backups**: `~/.agent-switch/skill-backups/` (vor der Deinstallation automatisch erstellt, behält die 20 neuesten)
...
> **Hinweis**: Agent Switch für macOS ist von Apple code-signiert und notarisiert. Sie können es direkt installieren und öffnen.
...
- **SSOT** (Single Source of Truth): Alle Daten werden in `~/.agent-switch/agent-switch.db` (SQLite) gespeichert
