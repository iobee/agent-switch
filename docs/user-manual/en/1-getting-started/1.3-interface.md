...
| 3 | Gateway Toggle | Start/stop the local Gateway service |
| 4 | App Switcher | Switch between Claude / Claude Desktop / Codex / Gemini / OpenCode / OpenClaw / Hermes |
| 5 | Status Area | Shows Gateway, usage, and update status |
...
- **Hermes** - Manage Hermes Agent provider configuration
...
| **Speed Test** | Shows loading animation during test | Only available when Gateway is running |
...
| **Gateway Active** | Green border | Provider actually in use during Gateway takeover mode |
| **Normal** | Default border | Inactive provider |
| **In Failover** | Shows priority badge | e.g., P1, P2 indicates failover priority |

### Health Status Badges

In Gateway mode, providers in the failover queue display health status:
