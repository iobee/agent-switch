...
The storage location for Agent Switch's own data, defaulting to `~/.agent-switch/`.
...
The exported file name format is `agent-switch-export-{timestamp}.sql`.
...
| Remote Directory | Remote storage path (default: `agent-switch-sync`) |
