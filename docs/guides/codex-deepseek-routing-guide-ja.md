...
プロバイダーを保存した後、Codex を再起動してください。Agent Switch は `agent-switch-model-catalog.json` を生成し、そのパスを `model_catalog_json` に書き込みますが、実行中の Codex プロセスがモデルカタログをホットロードするとは限りません。
