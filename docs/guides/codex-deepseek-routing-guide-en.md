...
Restart Codex after saving the provider. Agent Switch generates `agent-switch-model-catalog.json` and writes its path to `model_catalog_json`, but a running Codex process may not hot-load the model catalog.
