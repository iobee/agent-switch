...
### 🌐 唯一官方网站：**[github.com/iobee/agent-switch](https://github.com/iobee/agent-switch)**
...
<td>感谢 Cubence 赞助本项目！Cubence 是一家可靠高效的 API 中继服务提供商，提供对 Claude Code、Codex、Gemini 等模型的中继服务，并提供按量、包月等灵活的计费方式。Cubence 为 Agent Switch 的用户提供了特别优惠：使用 <a href="https://cubence.com/signup?code=CCSWITCH&source=ccs">此链接</a> 注册，并在充值时输入 "CCSWITCH" 优惠码，每次充值均可享受九折优惠！</td>
</tr>
...
<td>感谢 Crazyrouter 赞助了本项目！Crazyrouter 是一个高性能 AI API 聚合平台——一个 API Key 即可访问 300+ 模型，包括 Claude Code、Codex、Gemini CLI 等。全部模型低至官方定价的 55%，支持自动故障转移、智能路由和无限并发。Crazyrouter 为 Agent Switch 用户提供了专属优惠：通过<a href="https://crazyrouter.com/register?aff=OZcm&ref=cc-switch">此链接</a>注册后联系客服即可领取 <strong>$2 免费额度</strong>，首次充值时输入优惠码 `CCSWITCH` 还可获得额外 <strong>30% 奖励额度</strong>！</td>
</tr>
...
现代 AI 编程经常需要在不同模型、供应商和协议之间切换。Claude Code、Claude Desktop、Codex、Gemini CLI、OpenCode、OpenClaw 和 Hermes 等客户端各有配置格式，直接修改 JSON、TOML 或 `.env` 文件既慢又容易出错。

**Agent Switch** 提供一个本地桌面控制台，在本机运行 AI API Gateway，统一管理上游供应商、模型路由、协议适配、用量监控和故障转移。你可以通过可视化界面添加 50+ 供应商预设、一键切换模型/供应商、通过托盘快速控制路由，并用 SQLite、原子写入和自动备份保护本地配置。

- **本地 AI API Gateway** — 为 Claude、Codex、Gemini 等客户端提供统一本地入口
- **模型与供应商切换** — 50+ 供应商预设，包括官方服务、云平台和社区中转服务
- **协议适配与路由** — 在支持场景下进行 Claude / OpenAI / Gemini 请求适配和应用接管
- **用量监控** — 追踪请求数、Token、成本、延迟、错误和供应商表现
- **故障转移** — 自动故障转移、熔断器、供应商健康监控和备用队列
- **系统托盘快速控制** — 从托盘菜单即时切换供应商，无需打开完整应用
- **同步与备份** — 通过自定义配置目录、WebDAV 或 S3 在不同设备之间同步供应商数据
...
- **本地 Gateway 热切换** — 格式转换、自动故障转移、熔断器、供应商健康监控和整流器
- **应用级接管** — 独立为 Claude、Codex 或 Gemini 配置 Gateway，具体到单个供应商
...
- **Deep Link** (`agentswitch://`) — 通过 URL 一键导入供应商配置
...
Agent Switch macOS 版本已通过 Apple 代码签名和公证，可直接下载安装，无需额外操作。推荐使用 `.dmg` 安装包。
...
- **数据库**：`~/.agent-switch/agent-switch.db`（SQLite — 供应商、Gateway 状态、用量与配置数据）
- **本地设置**：`~/.agent-switch/settings.json`（设备级 UI 偏好设置）
- **备份**：`~/.agent-switch/backups/`（自动轮换，保留最近 10 个）

</details>

## 文档

如需了解各项功能的详细使用方法，请查阅 **[用户手册](docs/user-manual/zh/README.md)** — 涵盖供应商管理、本地 Gateway、故障转移、用量统计、同步与备份等功能。
...
- **Gateway**：在设置中启动本地 Gateway，并按应用开启接管
- **用量**：在用量页查看请求数、Token、成本、错误和延迟
- **故障转移**：为 Claude / Codex / Gemini 配置备用供应商队列和熔断策略
...
> **注意**：Agent Switch macOS 版本已通过 Apple 代码签名和公证，可直接安装打开。
...
- **SSOT**（单一事实源）：所有数据存储在 `~/.agent-switch/agent-switch.db`（SQLite）
...
- **ProxyService**：本地 Gateway 模式，支持热切换、格式转换和应用接管
- **FailoverService**：供应商健康状态、熔断器和备用队列
- **UsageService**：请求日志、Token、成本和模型用量统计
