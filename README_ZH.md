...
**CC Switch** 提供一个本地桌面控制台，在本机运行 AI API Gateway，统一管理上游供应商、模型路由、协议适配、用量监控和故障转移。你可以通过可视化界面添加 50+ 供应商预设、一键切换模型/供应商、通过托盘快速控制路由，并用 SQLite、原子写入和自动备份保护本地配置。
...
- **Deep Link** (`ccswitch://`) — 通过 URL 一键导入供应商配置
...
- **数据库**：`~/.cc-switch/cc-switch.db`（SQLite — 供应商、Gateway 状态、用量与配置数据）
