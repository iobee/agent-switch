...
### 🌐 唯一の公式サイト：**[github.com/iobee/agent-switch](https://github.com/iobee/agent-switch)**
...
<td>Cubence のご支援に感謝します！Cubence は Claude Code、Codex、Gemini などのリレーサービスを提供する信頼性の高い API 中継プラットフォームで、従量課金や月額プランなど柔軟な料金体系を提供しています。Agent Switch ユーザー向けの特別割引：<a href="https://cubence.com/signup?code=CCSWITCH&source=ccs">このリンク</a>で登録し、チャージ時に「CCSWITCH」クーポンを入力すると、毎回 10% オフになります！</td>
</tr>
...
Agent Switch の macOS 版は Apple によるコード署名と公証が完了しています。直接ダウンロードしてインストールできます — 追加の手順は不要です。`.dmg` インストーラの使用を推奨します。
...
- **データベース**: `~/.agent-switch/agent-switch.db`（SQLite -- プロバイダ、MCP、Prompts、Skills）
- **ローカル設定**: `~/.agent-switch/settings.json`（デバイスレベルの UI 設定）
- **バックアップ**: `~/.agent-switch/backups/`（自動ローテーション、最新 10 件を保持）
- **Skills**: `~/.agent-switch/skills/`（デフォルトでシンボリックリンクにより対応アプリに接続）
- **Skill バックアップ**: `~/.agent-switch/skill-backups/`（アンインストール前に自動作成、最新 20 件を保持）
...
- **SSOT** (Single Source of Truth): すべてのデータを `~/.agent-switch/agent-switch.db`（SQLite）に集約
